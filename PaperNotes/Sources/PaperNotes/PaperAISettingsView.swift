...
struct PaperAISettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if settings.provider == .server {
                ...
                Text("Keys are stored in macOS Keychain. Your selected paper excerpts and notes is sent to the server you choose when you generate lessons.")
            ...
            HStack {
                ...
                }.buttonStyle(BorderedProminentButtonStyle()).disabled(testing)
            }
        }
        .buttonStyle(BorderedButtonStyle())
        .textFieldStyle(.roundedBorder)
        .padding(24).frame(width: 580)
        .background(ReviewTheme.surface).foregroundStyle(Color.primary).tint(ReviewTheme.accent)
        .disabled(testing)
        .onAppear {
            guard previewSettings == nil else { return }
            do { settings = try PaperAI.load() }
    ...
    private func persist() throws {
        ...
        try settings.save(to: PaperAI.settingsURL)
    }
