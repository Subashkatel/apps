// Adapted from digimata/quill (MIT); see Resources/Quill-LICENSE.
import AVFoundation
import CoreAudio
import Foundation
...
final class SystemAudioRecorder {
    ...
    private let queue = DispatchQueue(label: "local.meetingnotes.system-tap")
    private(set) var isRecording = false
    /// Wall-clock time of the first captured buffer — the track's true start,
    /// used to offset-align the two tracks' transcript timestamps.
    private(set) var firstBufferAt: Date?

    /// Start capturing system audio as PCM CAF. AudioSink reports disk errors
    /// and keeps writes independent of transcription and summarization.
    func start(writingTo url: URL) throws {
        guard !isRecording else { return }

        let description = CATapDescription(stereoGlobalTapButExcludeProcesses: [])
        description.name = "Meeting Notes system audio"
        ...
        do {
            ...
            sink = try AudioSink(url: url, format: format)
    ...
    private func createAggregateDevice(tapUUID: UUID) throws {
        let desc: [String: Any] = [
            kAudioAggregateDeviceNameKey: "Meeting Notes audio",
