...
struct MeetingSettings:View {
    ...
    var body:some View {
        VStack(alignment:.leading,spacing:18){
            HStack{Text("Settings").font(.custom("Georgia",size:25));Spacer();Button("Done"){save()}.keyboardShortcut(.defaultAction)}
            ScrollView{VStack(alignment:.leading,spacing:24){
                ...
                GroupBox("AI summaries") {VStack(alignment:.leading,spacing:12){
                    ...
                    TextField("Model · leave blank for CLI default",text:model)
                    ...
                    Text("Only Summarize sends the transcript, your notes and selected project context to this provider. Recording is independent of AI. A CLI default is not an exact model name.").font(.caption).foregroundStyle(.secondary)
